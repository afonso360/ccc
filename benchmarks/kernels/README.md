...
variadic definition plus caller. A second algorithmic slice adds CRC32, dense
matrix multiply, heap sort, Dijkstra shortest paths, and an image stencil so
that code-generation changes are also visible in compact complete programs.
...
| `crc32` | 1,048,576 input bytes | Unsigned result `0xaf3f6d98`; eight branchless CRC steps consume every generated byte. |
| `matrix-multiply` | 1,048,576 unsigned multiply-accumulates | Unsigned result `0xfe6cbaf0`; 32 dependent 32x32 dense products update their inputs between passes. |
| `heap-sort` | 128 1,024-element sorts | Unsigned result `0x6236dd0a`; each randomly generated array is checked for sorted order. |
| `dijkstra` | 512 64-node shortest paths | Unsigned result `0xced1bd1d`; each complete weighted graph traversal selects and relaxes every node. |
| `image-stencil` | 1,016,064 five-point stencils | Unsigned result `0x2c970468`; 64 passes alternate between two 128x128 images. |
...
The algorithmic cases have no libc dependency: their inputs derive from
volatile seeds, array bounds are fixed and in range, and checksums include the
final mutable state. Heap sort additionally proves the generated ordering
before accepting its checksum; Dijkstra uses a complete positive-weight graph,
so every selected distance is finite. The image stencil preserves initialized
borders while alternating its two distinct image buffers.
