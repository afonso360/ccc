    ...
    !__has_builtin(__builtin_ctzll) || !__has_builtin(__builtin_popcount) || \
    !__has_builtin(__builtin_popcountll) || !__has_builtin(__builtin_prefetch)
#error "required integer builtins are unavailable"
#endif

#if __has_builtin(__builtin_bswap32) || __has_builtin(__builtin_ctz) || \
    __has_builtin(__builtin_ctzl) || __has_builtin(__builtin_popcountl)
...
static int folded_array[(folded_clz + folded_clzl + folded_clzll +
                         folded_ctzll + folded_popcount +
                         folded_popcountll == 177)
                            ? 7
                            : -1];
