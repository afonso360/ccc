...
long ccc_hfa_union_after_seven(
    double a0, double a1, double a2, double a3, double a4, double a5, double a6,
...
struct Pair ccc_pair_transform(struct Pair value) {
    value.first += 3;
    value.second += 5;
    return value;
}

struct Big ccc_big_transform(struct Big value) {
    value.first += 3;
    value.second += 5;
    value.third += 7;
    return value;
