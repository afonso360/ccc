...
| Storage classes           | `typedef`, `extern`, `static`, `auto`, `register`, `_Thread_local`, GNU `__thread`                                                    | Context rules are checked. Automatic, static, and thread storage durations are represented; block `extern` resolves to file scope.                                                                                                                                                                                                  |
...
| Type qualifiers           | `const`, `restrict`, `volatile`, `_Atomic`                                                                                           | Qualifiers remain on declarations and places; value conversion produces an unqualified rvalue type. `const` modification is rejected and volatile accesses are ordered. Naturally aligned 1-, 2-, 4-, and 8-byte integer and pointer atomic loads/stores are implemented; unsupported representations retain the `CCC4011` boundary. |
| Scalar types              | `void`, `_Bool`, plain/signed/unsigned `char`, signed/unsigned `short`, `int`, `long`, `long long`, `_Float16`, `float`, `double`, `long double` | Integer width, rank, signedness, promotions, and target plain-`char` behavior are implemented. `_Float16`, `float`, and `double` scalar operations are implemented. `_Float16` retains its exact IEEE binary16 payload in storage and SSA, rounds constants, conversions, and arithmetic to binary16, and uses the native half-width calling convention and variadic transport on every enabled target. The binary16 predefined-macro family is advertised. On x86-64 Linux, `long double` uses exact f80 constants, address-backed runtime values, x87 assembly helpers, and generated SysV boundary bridges. Linux binary128 profiles retain only representation and layout. |
...
represented on every place. Supported native scalar accesses are ordered and
atomic; aggregates, floating representations, 128-bit integers, weakened
alignment, and other unsupported forms fail closed instead of becoming
ordinary accesses.
...
| Arrays                      | Constant bounds, omitted bounds, `[*]`, runtime bounds, and parameter-array `static`/qualifiers                                                    | Prototype `[*]` and nonconstant bounds are distinct from incomplete arrays. Parameter, local-declaration, typedef, and evaluated type-name extents are retained exactly once; earlier parameters are visible to later bounds. Fixed-size pointer-to-VLA objects and arena-backed automatic VLA objects are implemented on every enabled target. GNU fixed zero-length arrays remain accepted, but an array type that combines a zero-length dimension with any runtime dimension is rejected with `CCC2456` before IR lowering. |
...
| Abstract declarators        | Pointers, arrays, and functions in casts, `sizeof`, `_Alignof`, parameters, and type names                                                         | Runtime bounds in evaluated casts and layout-dependent `sizeof` operands are retained and evaluated once. Bounds that cannot affect pointer `sizeof` or `_Alignof` are deliberately unevaluated.                                                                                                                                                                                                                                                                      |
...
| -------------------------- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Primary and literals       | Identifiers, including C11 `__func__` and the GNU-mode aliases `__FUNCTION__` and `__PRETTY_FUNCTION__`; integer, floating, character, and ordinary/`u8`/`u`/`U`/`L` string literals; parenthesized expressions | The three function-name spellings designate the same per-function static const array. Integer literal candidates follow radix and suffix rules. Ordinary character constants apply target plain-`char` signedness. Adjacent strings are concatenated before parsing. x86 f80 literals preserve their target precision and deterministic ABI padding. |
| Postfix                    | Subscript, direct and indirect calls, `.` and `->`, postfix `++`/`--`, and compound literals                        | Compound literals are addressable lvalues. A block-scope occurrence has one retained automatic object that is reinitialized when evaluated. Each file-scope occurrence becomes a distinct internal static object and uses the ordinary static initializer and relocation graph. Qualifiers and completed array bounds are retained in both cases.                         |
...
| Layout operators           | `sizeof expression`, `sizeof(type-name)`, `_Alignof(type-name)`, `__builtin_offsetof` with nested member/index paths | Compile-time layouts remain integer constant expressions. VLA `sizeof` lowers to an explicit checked runtime-size operation using saved extents. `_Alignof` strips array dimensions without evaluating them; `offsetof` rejects bit-fields and nonconstant indices.                                                                                                                                                               |
...
| Integer builtins           | `__builtin_bswap64`, `__builtin_clz`, `__builtin_clzl`, `__builtin_clzll`, `__builtin_ctz`, `__builtin_ctzll`, `__builtin_popcount`, and `__builtin_popcountll` | Use their exact GNU argument and result types and lower to native Cranelift `bswap`, `clz`, `ctz`, and `popcnt` operations. Count-leading/trailing-zero with a zero operand retains GNU's undefined-behavior contract. Other family spellings, including `__builtin_bswap32`, are not inferred. |
| Memory builtins            | `__builtin_memcpy`, `__builtin_memmove`, and `__builtin_memset`                                                    | Enforce the libc-compatible destination, source/value, and `size_t` conversions, return the converted destination, and lower to the target's corresponding memory libcall. `memmove` retains overlap-safe semantics. |
...
| Scalar atomic builtins     | `__atomic_load_n`, `__atomic_store_n`, `__atomic_exchange_n`, fetch/add/sub/and/or/xor old/new forms, `__atomic_compare_exchange_n`, and thread/signal fences | Naturally aligned 1-, 2-, 4-, and 8-byte integer objects and pointer load/store/exchange/CAS lower without external runtime symbols. Legal weaker orders are accepted and strengthened to sequential consistency; constant invalid load/store/failure orders are rejected. Pointer add/sub forms fail closed because GNU raw-byte and standard scaled semantics cannot be conflated. |
| Casts                      | Arithmetic conversions, pointer/integer casts, pointer/pointer casts, casts to `_Bool` and `void`                    | x86 f80 converts to and from `_Bool`, every enabled integer type, `float`, and `double`; 128-bit integer conversions use operation-selected compiler-runtime helpers. Linux binary128 value conversions remain unavailable. |
...
| Assignment                 | `=`, all arithmetic/shift/bitwise compound assignments, prefix/postfix increments                                    | Implemented with one evaluation of the target place. Aggregate assignment uses an overlap-safe copy; volatile aggregate reads and writes expand into ordered accesses. Native atomic integer add/sub/and/or/xor and scaled pointer add/sub/increment use atomic RMW instructions; other atomic compound operators fail closed. |
...
| Generic selection          | `_Generic` controlling conversions, association validation, and selection                                           | The controlling expression undergoes lvalue, array, and function conversion for type matching but is absent from executable IR. Association types must be distinct compatible complete non-variably-modified object types, with at most one default. The selected expression retains its type, place, bit-field descriptor, and constant-expression status.              |
...
| GNU statement expression   | `({ block-items })` in `gnu11`                                                                                       | Introduces a block scope, executes preceding items in order, and produces `void`, a captured value, or the narrowly transparent final lvalue described by the GNU semantics contract. |
...
only when used. In `gnu11`, `__FUNCTION__` and `__PRETTY_FUNCTION__` resolve to
that same object; strict `c11` exposes only the standard identifier.

GNU statement expressions are implemented with the result-category and scope
rules specified in the
[GNU C semantics contract](core-c11-and-gnu-semantics.md#statement-expressions).
GNU inline-assembly statements retain templates, operands, constraints,
clobbers, qualifiers, and goto labels through parsing. Semantic analysis admits
only the exact certified forms described in the
[GNU C semantics contract](core-c11-and-gnu-semantics.md#builtins-and-inline-assembly);
every other form fails before object emission. Declaration assembly labels are
the separate symbol-renaming construct listed above.
...
debug locations for TLS definitions use DTP-relative expressions on x86-64 ELF
and TLV-relative expressions on Darwin arm64. AArch64 and RISC-V ELF retain the
definition, type, and source metadata but omit the location until their TLS
DWARF relocation contracts have independent debugger evidence.
...
| Calls                   | Direct and function-pointer calls use the same module ABI plan. Supported scalar and aggregate fixed boundaries lower natively; `x86_64-unknown-linux-gnu` prototyped variadic, unprototyped, wide-integer, and f80 calls use generated bridges. f80 scalar returns use `%st(0)`, f80 arguments use aligned stack storage, and recursively containing aggregates use the SysV memory class. Linux binary128 boundaries fail before object emission. |
...
| State                     | Exact keys                                                                                                                                                                                                                  |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Implemented               | `__const`, `__const__`, `__inline`, `__inline__`, `__restrict`, `__restrict__`, `__signed`, `__signed__`, `__volatile`, `__volatile__`, `__alignof`, `__alignof__`, `__thread`, `gnu-declaration-asm-labels`, `gnu-function-name-aliases`, `gnu-statement-expressions`, `gnu-restrict-qualifiers` |
| Behavior-compatible no-op | `__extension__`, `gnu-extension-marker`                                                                                                                                                                                     |
| Parse-only                | `__asm`, `__asm__`, `__attribute`, `__attribute__`, `__typeof`, `__typeof__`, `gnu-alternative-keywords`, `gnu-attribute-specifiers`, `gnu-typeof`                                                                           |
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Implemented               | `noreturn`, `__noreturn__`, function-only `noinline`, `__noinline__`, `always_inline`, and `__always_inline__`, record-specifier `packed` and `__packed__`, `mode`, `__mode__`, `aligned`, `__aligned__`, `weak`, `__weak__`, `visibility`, `tls_model`, `__tls_model__`, and the all-pointer inline-anonymous-typedef subset of `transparent_union` and `__transparent_union__`; their control-flow, optimization-policy, type, layout, binding, visibility, TLS relocation, or calling effects are retained through the applicable frontend and CCC-IR stages. Conflicting `always_inline` and `noinline` declarations receive `CCC2457`. Other `packed` placements receive `CCC2432`; unsupported transparent-union forms receive `CCC2439`; applying `tls_model` to an object without thread storage duration receives `CCC2441`. |
| Behavior-compatible no-op | `nothrow`, `__nothrow__`, `pure`, `__pure__`, `const`, `__const__`, `malloc`, `__malloc__`, `format`, `__format__`, `nonnull`, `__nonnull__`, `warn_unused_result`, `__warn_unused_result__`, `unused`, `__unused__`, `deprecated`, `__deprecated__`, `may_alias`, `__may_alias__`, `alloc_size`, `__alloc_size__`; CCC emits no TBAA metadata or allocation-size optimization, so omitting those optimizer contracts does not change generated behavior. |
...
GNU `always_inline` and `noinline` are canonical function properties, including
...
`__builtin_clz`, `__builtin_ctz`, and `__builtin_popcount` accept `unsigned
int`; `__builtin_clzl` accepts `unsigned long`; and the `clzll`, `ctzll`, and
`popcountll` forms accept `unsigned long long`. Every count form
...
Valid integer constant-expression operands fold for all eight forms and remain
...
The memory builtins convert the destination to `void *`, the copy/move source
to `const void *`, and the extent to target `size_t`; `memset` converts its
fill value to `int`. The expression result is the converted destination.
CCC-IR distinguishes overlap-safe copy from non-overlapping copy and a fill
operation. Code generation uses Cranelift's target-aware memory-libcall
lowering, preserving the platform's normal `memcpy`, `memmove`, and `memset`
ABI rather than exposing compiler-specific helper symbols.
...
The exact `__atomic_*_n` scalar family uses the same native-width boundary.
Load and store become ordered CCC-IR memory effects; exchange, integer
add/sub/and/or/xor, and compare-exchange use native RMW/CAS instructions.
Compare-exchange updates the expected object only on failure. Weak requests may
use the stronger non-spurious implementation. Source memory-order expressions
are evaluated once; constant-invalid load, store, and failure orders are
rejected, while every legal weaker order (including consume) is conservatively
strengthened to sequential consistency. An address derived from a packed member
is rejected when its typed layout guarantees less alignment than the native
instruction requires, including provenance retained through casts, array
decay, pointer arithmetic, and conditional joins with otherwise unknown
pointer branches. An otherwise arbitrary pointer remains accepted:
its alignment is not knowable locally, and the source-level atomic contract
requires the caller to provide a naturally aligned object. GNU pointer
fetch-add/sub is not enabled, and the standard header therefore diagnoses
pointer fetch-add/sub rather than silently using GNU's unscaled byte arithmetic.
...
| --------------------------------------------------------------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Non-native atomic representations and operations                | Typed AST/CCC-IR or backend boundary       | Aggregates, floating and f80 values, 128-bit integers, atomic bit-fields, statically known weakened packed-member alignment, unsupported compound operators, and pointer fetch-add/sub builtins fail closed (`CCC2443`, `CCC2453`, `CCC2455`, or `CCC4011`). Unknown pointer provenance remains subject to C's naturally aligned object requirement. No non-atomic or libcall fallback is inferred. |
| Linux binary128 `long double` value operations and boundaries  | Layout/type representation                | `CCC2343`, `CCC2404`, or `CCC3509`; no implicit `double` substitution. x86 f80 value operations and boundaries are implemented through exact target constants, localized assembly, and operation-selected runtime helpers.                                                                                                                               |
...
| Unsupported inline-assembly forms                                | Untyped AST with complete statement spelling | Exact selected forms are certified; unsupported templates, constraints, clobbers, operand types, qualifiers, symbolic operands, and `asm goto` are rejected with `CCC2454` before object emission.                                                                                                                                                            |
...
shipped header inventory is five compiler-owned spelling
headers (`stdalign.h`, `stdatomic.h`, `stdbool.h`, `stdarg.h`, `stdnoreturn.h`), target-derived
`stddef.h`, the hosted `math.h` classification wrapper, and the Apple arm64
`sys/cdefs.h` inline-policy adaptation. The latter selects the SDK's own
static-inline fallback for header implementation functions; it is a compiler
resource adaptation, not a source-corpus patch.
