...
| Scalar types              | `void`, `_Bool`, plain/signed/unsigned `char`, signed/unsigned `short`, `int`, `long`, `long long`, `_Float16`, `float`, `double`, `long double` | Integer width, rank, signedness, promotions, and target plain-`char` behavior are implemented. `float` and `double` scalar operations are implemented. `_Float16` has its real two-byte layout so declaration-only SDK surfaces remain usable, but value initialization, arithmetic, calls, returns, and variadic fetches fail closed with `CCC3518`; no `__FLT16_*` capability macros are advertised. On x86-64 Linux, `long double` uses exact f80 constants, address-backed runtime values, x87 assembly helpers, and generated SysV boundary bridges. Linux binary128 profiles retain only representation and layout. |
...
| Arrays                      | Constant bounds, omitted bounds, `[*]`, runtime bounds, and parameter-array `static`/qualifiers                                                    | Prototype `[*]` and nonconstant bounds are distinct from incomplete arrays. Definition-parameter and local-declaration extents are retained once; earlier parameters are visible to later bounds. Fixed-size pointer-to-VLA objects and arena-backed automatic VLA objects are implemented. Runtime `sizeof` and several variably modified type-name/typedef contexts remain unavailable.                                           |
...
| --------------------------------------------------------------- | ----------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Remaining variably modified type contexts and runtime VLA layout | Typed declaration/type representation     | Prototype `[*]` is distinct; parameter and local-declaration bounds are retained, and automatic VLA objects lower through the hosted arena. Nonautomatic VLA objects are `CCC2258`; block `extern`, typedef, type-name, and block-function-declarator boundaries are `CCC2415`–`CCC2418`; runtime `sizeof` remains a layout boundary. |
...
| `_Float16` initialization, arithmetic, calls, returns, `va_arg` | Layout/type representation                | `CCC3518`; declaration-only prototypes and storage layout are supported, with no implicit `float` substitution or advertised `__FLT16_*` value capability                                                                                                                                    |
