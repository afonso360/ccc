...
checked emulated execution; the other fetched executable profiles currently
have audited runners only for `x86_64-unknown-linux-gnu`. The manifest catalog,
not the build host, is authoritative for each row.
