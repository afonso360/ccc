...
fn darwin_variadic_bridge_preserves_exact_public_assembly_label() {
    let config = EffectiveCompilationConfig::aarch64_apple_darwin();
    let output = emit_source_with_config(
        "int source_variadic(int fixed, ...) asm(\"_physical_variadic\");\n\
    ...
    let entry = primary.symbol_by_name("_physical_variadic").unwrap();
    assert!(entry.is_undefined());
    assert!(primary.symbol_by_name("__physical_variadic").is_none());
    ...
    assert!(assembly.source().contains(".globl _physical_variadic\n"));
    assert!(assembly.source().contains("_physical_variadic:\n"));
    assert!(!assembly.source().contains("__physical_variadic"));

    let manifest_entry = output
        .manifest
        .symbols()
        .iter()
        .find(|symbol| symbol.name == "_physical_variadic")
...
fn complete_abi_plan_and_aggregate_clif_have_exact_snapshots() {
    ...
    assert_eq!(
        sha256(&dump),
        "7cab8736ebd090c212dd4b16a2f0fcd8c97f22debf50d192256fca1add8f2d7b"
    );
