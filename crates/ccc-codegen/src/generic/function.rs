...
#[derive(Default)]
struct DirectAggregateParameterFieldLoads {
    loads: BTreeMap<gir::InstructionId, DirectAggregateParameterFieldLoad>,
    elided_backings: BTreeSet<gir::StorageId>,
}

/// Finds register-passed aggregate parameters whose backing storage is only
/// used to copy the entry value and load plain, directly projected scalar
/// fields.
///
...
/// produce. The proof follows nested record projections and rejects every
/// pointer use other than the entry copy, projection, or mapped scalar load.
fn direct_aggregate_parameter_field_loads(
    module: &gir::FullModule,
    function: &gir::FullFunction,
    config: &EffectiveCompilationConfig,
    plan: &ccc_abi::NativeBoundaryPlan,
) -> DirectAggregateParameterFieldLoads {
    let uses = gir::value_use_counts(function);
    ...
    let definitions = field_projection_definitions(function);
    let global_roots = field_projection_global_roots(function);
    let mut result = DirectAggregateParameterFieldLoads::default();
    'parameters: for (storage, parameter) in parameters {
        ...
        let mut pointers = addresses.iter().copied().collect::<BTreeSet<_>>();
        loop {
            let mut changed = false;
            for instruction in function.blocks.iter().flat_map(|block| &block.instructions) {
                let gir::FullInstructionKind::ProjectField { base, .. } = &instruction.kind else {
                    continue;
                };
                if pointers.contains(base) {
                    let Some(projected) = instruction.result else {
                        continue 'parameters;
                    };
                    changed |= pointers.insert(projected);
                }
            }
            if !changed {
                break;
        ...
        let mut expected_pointer_uses = pointers
            .iter()
            .copied()
            .map(|pointer| (pointer, 0_usize))
        ...
        for instruction in function.blocks.iter().flat_map(|block| &block.instructions) {
            match &instruction.kind {
                gir::FullInstructionKind::AggregateCopy {
                    ...
                    *expected_pointer_uses
                        .get_mut(destination)
                        .expect("aggregate copy destination is a parameter address") += 1;
                    expected_incoming_uses += 1;
                }
                gir::FullInstructionKind::ProjectField { base, .. } if pointers.contains(base) => {
                    *expected_pointer_uses
                        .get_mut(base)
                        .expect("projected parameter pointer is tracked") += 1;
                }
                gir::FullInstructionKind::Load {
                    address,
                    object,
                    ...
                    *expected_pointer_uses
                        .get_mut(address)
                        .expect("loaded parameter pointer is tracked") += 1;
                    if *access != gir::MemoryAccess::default() || !object.qualifiers.is_empty() {
                        continue 'parameters;
                    }
                    let (root, projections) =
                        field_projection_path(*address, &definitions, &global_roots);
                    let FieldProjectionRoot::Value(root) = root else {
                        continue 'parameters;
                    };
                    if !addresses.contains(&root) || projections.is_empty() {
                        continue 'parameters;
                    }
                    let Some((source_offset, size)) =
                        field_projection_offset(module, &projections, config)
                    else {
                        continue 'parameters;
                    };
        ...
        if expected_incoming_uses == 0
            ...
            || expected_pointer_uses
                .iter()
                .any(|(pointer, expected)| uses[pointer.0 as usize] != *expected)
            || loads.is_empty()
        {
            continue;
        }
        result.loads.extend(loads);
        result.elided_backings.insert(storage);
    }
    result
}

#[derive(Clone, Copy)]
struct DirectAggregateReturnFieldStore {
    value: gir::ValueId,
}

#[derive(Default)]
struct DirectAggregateReturnFieldStores {
    return_values: BTreeMap<gir::ValueId, BTreeMap<u32, DirectAggregateReturnFieldStore>>,
    elided_backings: BTreeSet<gir::StorageId>,
    stores: BTreeSet<gir::InstructionId>,
}

/// Finds automatic aggregate return objects whose complete final value comes
/// from plain scalar field stores with exact register-result carriers.
///
/// The storage has no observable read when its trailing snapshot feeds the
/// enclosing return, so the stores and later carrier loads can both forward
/// their scalar values directly to the ABI return.
fn direct_aggregate_return_field_stores(
    module: &gir::FullModule,
    function: &gir::FullFunction,
    config: &EffectiveCompilationConfig,
    plan: &ccc_abi::NativeBoundaryPlan,
) -> DirectAggregateReturnFieldStores {
    let ccc_abi::NativeResultPlan::RegisterAggregate { classified, .. } = &plan.result else {
        return DirectAggregateReturnFieldStores::default();
    };
    if classified.size != align_up_u64(classified.size, 8).unwrap_or(u64::MAX)
        || !native_result_carriers_fully_overwrite_storage(&plan.clif_results, classified.size)
            .unwrap_or(false)
    {
        return DirectAggregateReturnFieldStores::default();
    }
    ...
    let mut result = DirectAggregateReturnFieldStores::default();
    'returns: for block in &function.blocks {
        let Some(snapshot) = block.instructions.last() else {
            continue;
        };
        let Some(returned) = block
            .terminator
            .as_ref()
            .and_then(|terminator| match terminator {
                gir::FullTerminator::Return(Some(value)) => Some(*value),
                _ => None,
            })
        else {
            continue;
        ...
        let (
            Some(snapshot_value),
            gir::FullInstructionKind::AggregateSnapshot {
                source,
                object,
                access,
            },
        ...
        if snapshot_value != returned
            || uses[snapshot_value.0 as usize] != 1
            || *access != gir::MemoryAccess::default()
        {
            continue;
        }
        let Ok(snapshot_layout) = object_layout(&module.types, *object, config) else {
            continue;
        };
        if snapshot_layout.size != classified.size {
            continue;
        }
        let Some(&(source_block, storage)) = automatic_addresses.get(source) else {
            continue;
        };
        ...
        *expected_address_uses
            .get_mut(source)
            .expect("return snapshot source is an automatic address") += 1;
        ...
        for (field_address, (source_offset, end)) in fields {
            ...
            let mut matching_store = None;
            for instruction in &block.instructions {
                let gir::FullInstructionKind::Store {
                    address,
                    value,
                    object,
                    access,
                ...
                if *access != gir::MemoryAccess::default() || !object.qualifiers.is_empty() {
                    if function.value_types.get(value.0 as usize) != Some(&object.ty) {
                        continue 'returns;
                    }
                    continue 'returns;
                }
                ...
                if matching_store
                    .replace((
                        carrier_index,
                        instruction.id,
                        DirectAggregateReturnFieldStore { value: *value },
                    ))
                    .is_some()
                {
                    continue 'returns;
            ...
            let Some((carrier_index, store, descriptor)) = matching_store else {
                continue 'returns;
            };
            if return_values.insert(carrier_index, descriptor).is_some() {
                continue 'returns;
            }
            stores.insert(store);
        }
        if return_values.len() != plan.clif_results.len()
            || plan
                .clif_results
                .iter()
                .enumerate()
                .any(|(index, _)| !return_values.contains_key(&(index as u32)))
        {
            continue;
        }
        result.return_values.insert(snapshot_value, return_values);
        result.elided_backings.insert(storage);
...
/// A pointer base followed by a direct sequence of record-field projections.
///
/// Equal paths identify the same subobject without assuming anything about
/// arithmetic-derived pointers or aliases.
#[derive(Clone, Copy, Eq, PartialEq)]
enum FieldProjectionRoot {
    Value(gir::ValueId),
    Global(u32),
}

type FieldProjectionPath = (FieldProjectionRoot, Vec<(QualifiedType, usize)>);

fn field_projection_definitions(
    function: &gir::FullFunction,
) -> BTreeMap<gir::ValueId, (gir::ValueId, QualifiedType, usize)> {
    function
        .blocks
        .iter()
        .flat_map(|block| &block.instructions)
        .filter_map(|instruction| {
            ...
            instruction
                .result
                .map(|result| (result, (*base, *record, *field_index)))
        })
        .collect()
}

fn field_projection_global_roots(function: &gir::FullFunction) -> BTreeMap<gir::ValueId, u32> {
    function
        .blocks
        .iter()
        .flat_map(|block| &block.instructions)
        .filter_map(|instruction| {
            let gir::FullInstructionKind::AddressOfGlobal { global } = &instruction.kind else {
                return None;
            };
            instruction.result.map(|result| (result, global.0))
        })
        .collect()
}

fn field_projection_path(
    value: gir::ValueId,
    definitions: &BTreeMap<gir::ValueId, (gir::ValueId, QualifiedType, usize)>,
    global_roots: &BTreeMap<gir::ValueId, u32>,
) -> FieldProjectionPath {
    let mut root = value;
    let mut fields = Vec::new();
    while let Some(&(base, record, index)) = definitions.get(&root) {
        fields.push((record, index));
        root = base;
    }
    fields.reverse();
    let root = global_roots.get(&root).copied().map_or(
        FieldProjectionRoot::Value(root),
        FieldProjectionRoot::Global,
    );
    (root, fields)
}

fn field_projection_offset(
    module: &gir::FullModule,
    fields: &[(QualifiedType, usize)],
    config: &EffectiveCompilationConfig,
) -> Option<(u64, u64)> {
    let mut offset = 0_u64;
    let mut size = 0_u64;
    for (record, index) in fields {
        let layout = object_layout(&module.types, *record, config).ok()?;
        let LayoutShape::Record(record_layout) = layout.shape else {
            return None;
        };
        let field = record_layout.fields.get(*index)?;
        if field.bitfield.is_some() {
            return None;
        }
...
/// Finds field reads immediately following a register-aggregate call result
/// copied to a destination and a single straight-line successor.
///
/// The result is still stored, preserving the destination update. The mapped
/// reads occur before any subsequent store or call and therefore observe the
/// same exact carrier bytes without reloading them from memory.
fn direct_aggregate_result_successor_field_loads(
    module: &gir::FullModule,
    function: &gir::FullFunction,
    config: &EffectiveCompilationConfig,
) -> BTreeMap<gir::InstructionId, DirectAggregateResultFieldLoad> {
    ...
    let global_roots = field_projection_global_roots(function);
    let uses = gir::value_use_counts(function);
    let mut predecessors = BTreeMap::<gir::BlockId, usize>::new();
    for block in &function.blocks {
    ...
    let mut result = BTreeMap::new();
    for block in &function.blocks {
        let Some(gir::FullTerminator::Branch(edge)) = &block.terminator else {
            continue;
        };
        let Some(successor) = function
            .blocks
            .iter()
        ...
        if !successor.parameters.is_empty() || predecessors.get(&successor.id) != Some(&1) {
            continue;
        }
        for (copy_index, pair) in block.instructions.windows(2).enumerate() {
            let (call, copy) = (&pair[0], &pair[1]);
            if copy_index + 2 != block.instructions.len() {
                continue;
            }
            let Some(call_result) = call.result else {
                continue;
            ...
            let (destination_root, destination_fields) =
                field_projection_path(*destination, &definitions, &global_roots);
            for instruction in &successor.instructions {
                ...
                let (root, fields) = field_projection_path(*address, &definitions, &global_roots);
                if !fields.starts_with(&destination_fields) {
                    continue;
                }
                let tail = &fields[destination_fields.len()..];
                if root != destination_root || tail.is_empty() {
                    continue;
                }
                let Some((source_offset, field_size)) =
                    field_projection_offset(module, tail, config)
                else {
                    continue;
                };
                let Ok(layout) = object_layout(&module.types, *object, config) else {
                ...
                result
                    ...
                    .or_insert(DirectAggregateResultFieldLoad {
                        call: call.id,
                        source_offset,
                        clif_type,
                        elides_result_stores: false,
                    });
            }
        }
    }
...
pub(super) fn lower_function(
    ...
    let DirectAggregateParameterFieldLoads {
        loads: direct_aggregate_parameter_field_loads,
        elided_backings: direct_aggregate_parameter_backing_elisions,
    ...
    .then(|| match definition_plan {
        ...
        DefinitionAbi::Variadic(_) => DirectAggregateParameterFieldLoads::default(),
    })
    .unwrap_or_default();
    let direct_aggregate_return_field_stores = (!collect_debug_values
        && matches!(
            config.optimization,
            OptimizationLevel::O2 | OptimizationLevel::O3
        ))
    .then(|| match definition_plan {
        DefinitionAbi::Native(plan) => {
            direct_aggregate_return_field_stores(module, function, config, plan)
        }
        DefinitionAbi::Variadic(_) => DirectAggregateReturnFieldStores::default(),
    })
    .unwrap_or_default();
    let mut elided_aggregate_backings = direct_aggregate_parameter_backing_elisions;
    elided_aggregate_backings.extend(
        direct_aggregate_return_field_stores
            .elided_backings
            .iter()
            .copied(),
    );
    ...
    for object in &function.storage {
        ...
        if elided_aggregate_backings.contains(&object.id) {
            continue;
        }
    ...
    let mut direct_aggregate_result_field_loads = (!collect_debug_values)
        ...
        .unwrap_or_default();
    if !collect_debug_values {
        direct_aggregate_result_field_loads.extend(direct_aggregate_result_successor_field_loads(
...
struct FunctionState<'a, 'references, 'object> {
    ...
    direct_aggregate_return_field_stores: DirectAggregateReturnFieldStores,
    direct_aggregate_parameter_field_loads:
        BTreeMap<gir::InstructionId, DirectAggregateParameterFieldLoad>,
    elided_aggregate_backings: BTreeSet<gir::StorageId>,
    ...
    fn bind_native_entry_parameters(
        ...
        for parameter in plan.parameters.clone() {
            ...
            let value = if parameter.classified.passing == ccc_abi::PassingMode::Scalar {
                ...
                let backing_elided = self.function.parameters[source]
                    .storage
                    .is_some_and(|storage| self.elided_aggregate_backings.contains(&storage));
                ...
                let (address, uses_caller_backing) = if let Some(storage) = caller_storage {
                    ...
                    (address, false)
                } else if backing_elided && forwards_register_fields {
                    let storage = self.function.parameters[source]
                        .storage
                        .expect("elided aggregate backing belongs to a parameter storage");
                    let address = builder.ins().iconst(ir::types::I64, 0);
                    self.cached_storage_addresses.insert(storage.0, address);
                    (address, false)
                } else {
                    (
        ...
        let lowered = (|| {
            ...
            match &instruction.kind {
                ...
                I::AddressOfStorage { storage } => {
                    ...
                    if self.elided_aggregate_backings.contains(storage) {
                        return Ok(Some(builder.ins().iconst(ir::types::I64, 0)));
                    }
        ...
        match &plan.result {
            ...
            ccc_abi::NativeResultPlan::RegisterAggregate { classified, .. } => {
                                    ...
                                    |(carrier, value)| {
                                        (carrier.source_offset == descriptor.source_offset
                                            ...
                                            .then_some((
                                                *load,
                                                *value,
                                                descriptor.elides_result_stores,
                                            ))
                                    },
                                )
                            })
                ...
                if let Some(values) = direct_field_load_values {
                    let elides_result_stores = values
                        .iter()
                        .all(|(_, _, elides_result_stores)| *elides_result_stores);
                    self.direct_aggregate_result_load_values
                        .borrow_mut()
                        .extend(values.into_iter().map(|(load, value, _)| (load, value)));
                    if elides_result_stores {
                        return Ok(Some(address));
                    }
                }
    ...
    fn lower_native_return(
        ...
        match (&plan.result, value) {
            ...
            (ccc_abi::NativeResultPlan::RegisterAggregate { classified, .. }, Some(value)) => {
                if let Some(return_values) = self
                    .direct_aggregate_return_field_stores
                    .return_values
                    .get(&value)
                {
                    let mut results = Vec::with_capacity(plan.clif_results.len());
                    for (index, _) in plan.clif_results.iter().enumerate() {
                        let descriptor = return_values
                            .get(&(index as u32))
                            .expect("return-field analysis covers every ABI carrier");
                        results.push(self.value(descriptor.value)?);
                    }
                    self.release_runtime_storage(builder)?;
